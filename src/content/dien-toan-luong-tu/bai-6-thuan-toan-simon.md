...
<div style="background-color: #f8fafc; border-left: 4px solid #3b82f6; padding: 16px; border-radius: 4px; margin: 20px 0;">
  <b>Bài toán Simon:</b><br/>
  Cho một hàm $f$ biến đổi chuỗi nhị phân độ dài $n$ thành một chuỗi nhị phân độ dài $n$:
  <br/><br/>
  <div style="text-align: center;">$f: \{0,1\}^n \rightarrow \{0,1\}^n$</div>
  <br/>
  Hàm $f$ được bảo đảm (promise) sẽ rơi vào một trong hai trường hợp sau:
  <ol>
    <li><b>Hàm 1-1 (Injective):</b> Mỗi đầu vào cho ra một đầu ra duy nhất.</li>
    <li><b>Hàm 2-1 (Two-to-one):</b> Tồn tại một chuỗi nhị phân ẩn $s \in \{0,1\}^n$ với $s \neq 0^n$, sao cho với mọi $x, y \in \{0,1\}^n$ ($x \neq y$), ta có:<br/>
    $f(x) = f(y)$ khi và chỉ khi $x = y \oplus s$</li>
  </ol>
  <i>(Trong đó, $\oplus$ là phép toán XOR từng bit mà ta đã học ở bài trước - bitwise XOR).</i><br/><br/>
  <b>Nhiệm vụ:</b> Tìm ra chuỗi ẩn $s$. Nếu hàm là 1-1, ta ngầm định $s = 0^n$.
</div>
